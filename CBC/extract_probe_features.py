...
parser.add_argument("--model",        type=str,   default="OpenGVLab/InternVL2-2B")
...
def encode_inputs(image, question, answer=None):
    ...
    # --------------------------------------------------------
        ...
        # InternVL2 expects <image> placeholder expanded to
        # <img><IMG_CONTEXT>*N</img>  where N = num_image_token * num_patches
        IMG_START   = "<img>"
        IMG_END     = "</img>"
        ...
        # Tell the model which token id is the image-context placeholder
        if getattr(model, "img_context_token_id", None) is None:
            model.img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT)

        system_msg = "You are an AI assistant whose name is InternVL."
...
for _, row in tqdm(df_all.iterrows(), total=len(df_all)):
    ...
    try:
        ...
        if HIDDEN_STATE == "prompt":
            ...
            feature_mask    = fwd_kwargs.get("attention_mask")
            ...
            feature_mask = full_kwargs.get("attention_mask")
            ...
            if POOLING == "last":
                ...
                if HIDDEN_STATE == "output":
                    ...
                    if feature_mask is not None:
                        m   = feature_mask[0].bool()
                        vec = h[m].float().mean(dim=0).cpu().numpy()
                    else:
